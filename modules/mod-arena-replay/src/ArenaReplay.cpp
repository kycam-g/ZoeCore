...
public:
    ...
    void OnBattlegroundUpdate(Battleground* bg, uint32 /* diff */) override
    {
        ...
        if (match.packets.empty() || bg->GetPlayers().empty())
        {
            ...
            {
                Player* replayPlayer = bg->GetPlayers().begin()->second;

                // ZoeCore fix:
                // O replay usa uma BG/Arena falsa para assistir. Ao finalizar, não pode aplicar desertor.
                // Colocar STATUS_WAIT_LEAVE antes de LeaveBattleground evita que a core trate como abandono real.
                if (sConfigMgr->GetOption<bool>("ArenaReplay.Zoe.NoDeserterOnReplayEnd.Enable", true) &&
                    sConfigMgr->GetOption<bool>("ArenaReplay.Zoe.NoDeserterOnReplayEnd.SetWaitLeaveStatus", true))
                {
                    bg->SetStatus(STATUS_WAIT_LEAVE);
                }

                replayPlayer->LeaveBattleground(bg);

                // Segurança extra para cores/mods custom que ainda apliquem a aura 26013.
                if (sConfigMgr->GetOption<bool>("ArenaReplay.Zoe.NoDeserterOnReplayEnd.Enable", true) &&
                    sConfigMgr->GetOption<bool>("ArenaReplay.Zoe.NoDeserterOnReplayEnd.RemoveDeserterAura", true))
                {
                    replayPlayer->RemoveAurasDueToSpell(26013);
                }
            }

            if (sConfigMgr->GetOption<bool>("ArenaReplay.Zoe.NoDeserterOnReplayEnd.Enable", true) &&
                sConfigMgr->GetOption<bool>("ArenaReplay.Zoe.NoDeserterOnReplayEnd.CleanupReplayData", true))
            {
    ...
    void saveReplay(Battleground* bg, TeamId winnerTeamId)
    {
        ...
        for (const auto& playerPair : bg->GetPlayers())
        {
            ...
            ChatHandler(player->GetSession()).PSendSysMessage("Replay salvo. ID da partida: {}", replayfightid + 1);
        }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        {
            ChatHandler(player->GetSession()).SendSysMessage("Arena Replay desativado!");
            return true;
        }
        ...
        if (isArena1v1Enabled)
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 1v1 dos ultimos 30 dias", GOSSIP_SENDER_MAIN, REPLAY_LATEST_1V1);

        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 2v2 dos ultimos 30 dias", GOSSIP_SENDER_MAIN, REPLAY_LATEST_2V2);
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 3v3 dos ultimos 30 dias", GOSSIP_SENDER_MAIN, REPLAY_LATEST_3V3);

        if (isArena3v3soloQEnabled)
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays Solo 3v3 dos ultimos 30 dias", GOSSIP_SENDER_MAIN, REPLAY_LATEST_3V3SOLO);

        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 5v5 dos ultimos 30 dias", GOSSIP_SENDER_MAIN, REPLAY_LATEST_5V5);

        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Assistir replay por ID", GOSSIP_SENDER_MAIN, REPLAY_MATCH_ID, "", 0, true);             // maybe add command .replay 'replayID' aswell
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Buscar replays por nome do jogador", GOSSIP_SENDER_MAIN, REPLAY_LIST_BY_PLAYERNAME, "", 0, true); // to do: show a list, showing games with type, teamname and teamrating
        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, "Meus replays favoritos", GOSSIP_SENDER_MAIN, MY_FAVORITE_MATCHES);                   // To do: somehow show teamName/TeamRating/Classes (it's a different db table)

        if (isArena1v1Enabled)
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 1v1 de todos os tempos", GOSSIP_SENDER_MAIN, REPLAY_TOP_1V1_ALLTIME);

        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 2v2 de todos os tempos", GOSSIP_SENDER_MAIN, REPLAY_TOP_2V2_ALLTIME);
        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 3v3 de todos os tempos", GOSSIP_SENDER_MAIN, REPLAY_TOP_3V3_ALLTIME);

        if (isArena3v3soloQEnabled)
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays Solo 3v3 de todos os tempos", GOSSIP_SENDER_MAIN, REPLAY_TOP_3V3SOLO_ALLTIME);

        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Top replays 5v5 de todos os tempos", GOSSIP_SENDER_MAIN, REPLAY_TOP_5V5_ALLTIME);


        AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Replays mais assistidos de todos os tempos", GOSSIP_SENDER_MAIN, REPLAY_MOST_WATCHED_ALLTIME);  // To Do: show arena type + watchedTimes, maybe hide team name
    ...
    bool OnGossipSelectCode(Player* player, Creature* /* creature */, uint32 /* sender */, uint32 action, const char* code) override
    {
        ...
        // Forbidden: ', %, and , (' causes crash when using 'Buscar replays por nome do jogador')
        ...
        switch (action)
        {
            case REPLAY_MATCH_ID:
            {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("ID de partida invalido.");
                    CloseGossipMenuFor(player);
                    return false;
                }
            ...
            case REPLAY_LIST_BY_PLAYERNAME:
            {
                ...
                if (!playerData)
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("Nenhum jogador encontrado com o nome: {}", std::string(code));
                    CloseGossipMenuFor(player);
                    return false;
                }
                ...
                if (result)
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("Replays encontrados para o jogador: {}", std::string(code));
                    do
                    {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("Nenhum replay encontrado para o jogador: {}", std::string(code));
                    CloseGossipMenuFor(player);
                    return false;
                }
            }
...
private:
    ...
    void ShowReplays(Player* player, Creature* creature, std::vector<ReplayInfo> matchInfos) {
        if (matchInfos.empty())
            AddGossipItemFor(player, GOSSIP_ICON_TAXI, "Nenhum replay encontrado.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        else
        {
    ...
    {
        AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Favoritar replay por ID", GOSSIP_SENDER_MAIN, MY_FAVORITE_MATCHES, "", 0, true);
        ...
        if (!result)
            AddGossipItemFor(player, GOSSIP_ICON_TAXI, "Nenhum replay favorito encontrado.", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
        else
        {
            do
            {
                ...
                const uint32 actionOffset = GOSSIP_ACTION_INFO_DEF + 30;
                AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "Assistir replay " + std::to_string(matchId), GOSSIP_SENDER_MAIN, actionOffset + matchId);
    ...
    void FavoriteMatchId(uint64 playerGuid, uint32 code)
    {
        ...
        if (result)
        {
            ...
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Replay ID {} salvo nos favoritos.", code);
                CloseGossipMenuFor(player);
            }
        }
        else
        {
            if (Player* player = ObjectAccessor::FindPlayer(ObjectGuid::Create<HighGuid::Player>(playerGuid)))
            {
                ChatHandler(player->GetSession()).PSendSysMessage("Replay ID {} nao existe.", code);
                CloseGossipMenuFor(player);
            }
        }
    }
    ...
    bool replayArenaMatch(Player* player, uint32 replayId)
    {
        ...
        {
            handler.PSendSysMessage("Voce nao pode estar em fila de arena ou BG.");
            return false;
        }
        ...
        if (!bg)
        {
            handler.PSendSysMessage("Nao foi possivel criar o mapa da arena!");
        ...
        handler.PSendSysMessage("Replay ID {} iniciado.", replayId);
    ...
    bool loadReplayDataForPlayer(Player* p, uint32 matchId)
    {
        ...
        if (!result)
        {
            ChatHandler(p->GetSession()).PSendSysMessage("Dados do replay nao encontrados.");
