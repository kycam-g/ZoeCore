...
enum CustomDamage
{
    DAMAGE_CHAIN_LIGHTNING = 500000,
    DAMAGE_ARCANE_SHOCK = 350000
};
        ...
        ScheduleTimedEvent(5s, [&]
            {
                if (IsHeroic())
                {
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0.0f, true))
                        me->CastCustomSpell(SPELL_CHAIN_LIGHTNING, SPELLVALUE_BASE_POINT0,
                            DAMAGE_CHAIN_LIGHTNING, target, false);
                }
                else
        ...
        ScheduleTimedEvent(5s, [&]
            {
                ...
                {
                    if (Unit* target = SelectTarget(SelectTargetMethod::Random, 0, 0.0f, true))
                        me->CastCustomSpell(SPELL_ARCANE_SHOCK, SPELLVALUE_BASE_POINT0,
                            DAMAGE_ARCANE_SHOCK, target, false);
                }
                else
